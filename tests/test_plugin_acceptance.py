...
class TestQuestionLogic(unittest.TestCase):
    ...
    def test_05_language_id_not_hardcoded(self):
        ...
        hardcoded_patterns = [
            r"'language_id'\s*=>\s*\d+",        # PHP array
            r'"language_id"\s*=>\s*\d+',         # PHP array double quotes
            r'"language_id"\s*:\s*\d+',          # JSON / другой формат
        ]
    ...
    def test_06_curl_error_handling(self):
        ...
        has_error_check = (
            ...
            "try" in self.content  # для случая если используется обёртка
        )
